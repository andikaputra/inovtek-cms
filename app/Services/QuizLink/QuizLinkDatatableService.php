...
class QuizLinkDatatableService
{
    public function __construct(
        private readonly QuizLinkQueryService $quizLinkQueryService
    ) {}

    public function quizLinkDatatable(Request $request, string $id_provinsi): JsonResponse
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($item) {
                ...
                $routeDelete = route('admin.home.detail.kuis.delete', ['id_provinsi' => $item->region->slug, 'id' => $item->id]);
                $routeEdit = route('admin.home.detail.kuis.edit', ['id_provinsi' => $item->region->slug, 'id' => $item->id]);
                $routeQuiz = route('admin.home.detail.kuis.registrant.index', ['id_provinsi' => $item->region->slug, 'id' => $item->id]);
                ...
                $element .= '<a data-id ="'.$item->id.'"  href="'.$routeQuiz.'" data-bs-toggle="tooltip" data-bs-placement="top" type="button" class="btn btn-warning text-white btn-sm mt-1 mx-1" title="Daftar Registrasi Peserta">
            ...
            ->addColumn('link', function ($item) {
                return isset($item->quiz_link) ? '<a href="'.$item->quiz_link.'" target="_blank" rel="noopener noreferrer">'.$item->quiz_link.'</a>' : null;
            })
            ->addColumn('status', function ($item) use ($id_provinsi) {
                $element = '';
                $checkExistActiveQuiz = $this->quizLinkQueryService->checkExistActive(id_provinsi: $id_provinsi);
                if ($item->is_active || ! $checkExistActiveQuiz) {
                    if ($item->is_active) {
                        $query = 'on';
                    } else {
                        $query = ' ';
                    }
                    $routeUpdate = route('admin.home.detail.kuis.update.active', ['id_provinsi' => $item->region->slug, 'id' => $item->id]);
    ...
    private function _filterDatatable(Request $request, Builder $query): Builder
    {
        ...
        if (isset($searchKeyword)) {
            $query->where(function ($query) use ($searchKeyword) {
                ...
                $query->whereRaw('LOWER(quiz_link) LIKE ?', ["%{$lowerKeyword}%"])
        ...
        if ($order_column_index == 3) {
            $query->orderBy('quiz_link', $order_column_dir);
        }
